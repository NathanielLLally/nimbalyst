...
exports.sendInformationalEmail = sendInformationalEmail;
 ...
 */
async function sendViaSMTPNative(config, to, subject, html, text) {
    const debug = false;
    try {
 ...
 */
async function sendViaRawSMTPS(config, to, subject, html, text, messageId) {
    try {
 ...
 * Send followup email
 */
async function sendFollowupEmail(contactName, email) {
...
Happy Tails Paw Care Team`;
    return sendEmail(email, 'Following Up - Let\'s Schedule a Call', textContent, textContent);
}
/**
 * Send informational email
 */
async function sendInformationalEmail(contactName, email, company, challenge) {
...
Best regards,
Happy Tails Paw Care Team`;
    return sendEmail(email, `We Got Your Message - ${challenge}`, textContent, textContent);
}
exports.default = {
    sendEmail,
    sendFollowupEmail,
